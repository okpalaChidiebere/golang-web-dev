...
Skill you need to have in your Web Dev Toolkit

- Hash based Message Authentication code(HMAC): What a Hash does is that give thesame input, a hash will produce thesame output. One reason we may want to use this is when we want to store some data(could be a message string, file, etc) on the users machine but then we want to verify that the user did not tamper with that data. We can take the value that we want to store in the users machine, run it through HMAC (our HMAC takes in a secret key as one of its params) which creates a hash and store the hash and the actual data in on our own machine(like a database or maybe on the client machine). Now, whenever the user sends the data back, we can take that data, run it through our hash algorithm again with our secret password which gives us a new hash. We will now compare the new hash number to the hash number that we have created originally with that data and if they match we know that the data send is thesame as the data we have and that the user did not tampered with that data.
- Base64 encoding: One of the reasons you may want to use this is maybe you want to jumble a string up so what you are storing is that much difficult for humans to read. In particular, another reason you may want to use it is to store json string values in the cookie. You cannot store json string in the cookie directly because cookies does not allow some certain characters to be stored in them and double quotes(") is one of them which exists in a JSON. But you can encode that JSON string and store it in the client's machine. When the client sends back the cookie, you decode it and get back your JSOn string
- Web Storage: Web storage is comprises of localStorage and sessions. Cookies could be one of the ways you can store stuff in the web but in some countries like Europe, you have to let the user know that you want to write to their cookies; some user will not allow that. Sessions and localStorage requires JavaScript and if there is not javascript or cookies, you will have to pass the value through the url as params securely using https. SessionStorage last so long for as far as that window browser is open and when the browser window is closed, what is in session storage goes away. LocalStorage still persists even when the browser closes. LocalStorage goes away when you explicitly delete them
- Context: With Context, you can associate it to a request, you can put some variables related to this request into that context(like sessionId, userIDs. Be careful about stuffing context with variables too much). You can use Context to know the context of a request through out your entire system like to be able to signal when a process is done(by creating a contextWithCancel and calling the cancel method explicitly) or timed out(creating a context with timeout) that every other process that is related to this context should cancel or to be able to put timeout on processes or work. This is something you should definitely use in your application at scale
- TLS & HTTPS: You can do secure communications between the client and the server. This means anything that leaves the client machine will be encrypted as it goes through the open plain internet wire, then the server can decrypt it whne the request comes. And the server will encrypt the message it is sending back to the client as well
- JavaScript Object Notation(JSON): It originally came out of Javascript but other programming languages has adopted it and go is no exception. It is used for sending data back and forth between machines. You encode data as JSON, send it over the wire, and the client receives it and decode it. This is commonly used with APIs where you send a request to the server, and the server response will be in json format. There are two ways to turn your data into JSON in go. One was is through Marshalling(takes go data structure and turns it into JSON) and UnMarshalling(takes JSON and turns it into go data structure). Another way is to Encode and Decode. You want to use marshal and unmarshal when want to take a go data structure, turn it into JSON and assign it to a variable. Encode takes a go data structure turns it into JSON and just sends it over the wire straight. Marshalling and Encode are all available at the encoding/json package. Use this (link)[https://mholt.github.io/json-to-go/] if you need to know the go data structure that you want to unmarshall a JSOn into. You can drop in any JSON and it will give you a go data structure representation of that json.
